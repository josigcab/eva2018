    ...
    public function store(Request $request)
    {
       ...
       $fecha_inicio=$this->parser($fecha_inicio);
        ...
        date_default_timezone_set('America/La_Paz');
         //date_default_timezone_set('UTC-04');
        // date_default_timezone_set('BO');
        $fecha_actual= date("Y-m-d H:i:s"); 

        //$fecha_actual = date("Y-m-d H:i:s", false, 'America/Caracas');
        #$fecha_actual= date(); 
        ...
        if($fecha_inicio < $fecha_actual){
         ...
         $mensajeA.='los datos de la fecha inicio no son validos'. $fecha_inicio." FIN ". $fecha_fin . "ACT". $fecha_actual ;
          $numero_preguntas = DB::table('preguntas')->where('examen_id', $id_examen)->get();
